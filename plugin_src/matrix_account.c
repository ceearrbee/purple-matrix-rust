...
void matrix_login(PurpleAccount *account) {
  ...
  if (g_getenv("MATRIX_RUST_INSECURE_SSL")) {
      purple_notify_warning(gc, "Security Warning", "Insecure TLS Mode Enabled", 
          "The MATRIX_RUST_INSECURE_SSL environment variable is set. "
          "TLS certificate verification is DISABLED. Your connection may be insecure.");
  }
  ...
  g_free(data_dir); 
  if (homeserver_derived) g_free(homeserver_derived);
}
...
static gboolean process_sso_cb(gpointer data) {
  ...
  if (account && purple_account_get_connection(account) != NULL) {
    ...
    char *msg = g_strdup_printf("A browser window should have opened to Matrix SSO. If not, please visit the URL below and paste the 'loginToken' here.\n\n%s", url);
    ...
    purple_debug_warning("matrix", "process_sso_cb: No Matrix account found to handle SSO!\n");
  }
...
static gboolean process_user_info_cb(gpointer data) {
  MatrixUserInfoData *d = (MatrixUserInfoData *)data; PurpleAccount *account = find_matrix_account_by_id(d->user_id);
  if (account && purple_account_get_connection(account) != NULL) {
...
void show_user_info_cb(const char *user_id, const char *display_name, const char *avatar_url, const char *target_user_id, bool is_online) {
  MatrixUserInfoData *d = g_new0(MatrixUserInfoData, 1); d->user_id = g_strdup(target_user_id); d->display_name = g_strdup(display_name); d->avatar_url = g_strdup(avatar_url); d->is_online = is_online;
  /* We use d->user_id to store the account context for now in MatrixUserInfoData, let's fix the struct too */
...
static gboolean process_room_preview_cb(gpointer data) {
  ...
  g_free(d->user_id); g_free(d->room_id_or_alias); g_free(d->html_body); g_free(d); return FALSE;
}

void room_preview_cb(const char *user_id, const char *room_id_or_alias, const char *html_body) {
  MatrixRoomPreviewData *d = g_new0(MatrixRoomPreviewData, 1); d->user_id = g_strdup(user_id); d->room_id_or_alias = g_strdup(room_id_or_alias); d->html_body = g_strdup(html_body);
  g_idle_add(process_room_preview_cb, d);
}

static gboolean process_chat_topic_cb(gpointer data) {
  MatrixTopicData *d = (MatrixTopicData *)data; PurpleAccount *account = find_matrix_account_by_id(d->user_id);
  if (account && purple_account_get_connection(account) != NULL) {
  ...
  g_free(d->user_id); g_free(d->room_id); g_free(d->topic); g_free(d->sender); g_free(d); return FALSE;
}

void chat_topic_callback(const char *user_id, const char *room_id, const char *topic, const char *sender) {
  MatrixTopicData *d = g_new0(MatrixTopicData, 1); d->user_id = g_strdup(user_id); d->room_id = g_strdup(room_id); d->topic = g_strdup(topic); d->sender = g_strdup(sender);
  g_idle_add(process_chat_topic_cb, d);
}

static gboolean process_chat_user_cb(gpointer data) {
  MatrixChatUserData *d = (MatrixChatUserData *)data; PurpleAccount *account = find_matrix_account_by_id(d->user_id);
  if (account && purple_account_get_connection(account) != NULL) {
    PurpleConversation *conv = purple_find_chat(purple_account_get_connection(account), get_chat_id(d->room_id));
    if (conv) {
      if (d->add) {
        if (!purple_conv_chat_find_user(PURPLE_CONV_CHAT(conv), d->member_id))
          purple_conv_chat_add_user(PURPLE_CONV_CHAT(conv), d->member_id, d->alias, PURPLE_CBFLAGS_NONE, FALSE);
      } else purple_conv_chat_remove_user(PURPLE_CONV_CHAT(conv), d->member_id, NULL);
    }
  }
  g_free(d->user_id); g_free(d->room_id); g_free(d->member_id); if (d->alias) g_free(d->alias); if (d->avatar_path) g_free(d->avatar_path); g_free(d); return FALSE;
}

void chat_user_callback(const char *user_id, const char *room_id, const char *member_id, bool add, const char *alias, const char *avatar_path) {
  MatrixChatUserData *d = g_new0(MatrixChatUserData, 1); d->user_id = g_strdup(user_id); d->room_id = g_strdup(room_id); d->member_id = g_strdup(member_id); d->add = add; d->alias = g_strdup(alias); d->avatar_path = g_strdup(avatar_path);
  g_idle_add(process_chat_user_cb, d);
}

static gboolean process_presence_cb(gpointer data) {
  MatrixPresenceData *d = (MatrixPresenceData *)data; PurpleAccount *account = find_matrix_account_by_id(d->user_id);
  if (account && purple_account_get_connection(account) != NULL) purple_prpl_got_user_status(account, d->target_user_id, d->is_online ? "available" : "offline", NULL);
  g_free(d->user_id); g_free(d->target_user_id); g_free(d); return FALSE;
}

void presence_callback(const char *user_id, const char *target_user_id, bool is_online) {
  MatrixPresenceData *d = g_new0(MatrixPresenceData, 1); d->user_id = g_strdup(user_id); d->target_user_id = g_strdup(target_user_id); d->is_online = is_online;
...
static gboolean process_sas_request_cb(gpointer data) {
  ...
  if (account && purple_account_get_connection(account) != NULL) {
    ...
    g_free(vd->user_id); g_free(vd->flow_id); g_free(vd);
  }
  return FALSE;
}

void sas_request_cb(const char *user_id, const char *target_user_id, const char *flow_id) {
  struct VerificationData *d = g_new0(struct VerificationData, 1); d->user_id = g_strdup(target_user_id); d->flow_id = g_strdup(flow_id);
...
static gboolean process_sas_emoji_cb(gpointer data) {
  ...
  g_free(ed->user_id); g_free(ed->flow_id); g_free(ed->emojis); g_free(ed); return FALSE;
}

void sas_emoji_cb(const char *user_id, const char *target_user_id, const char *flow_id, const char *emojis) {
  MatrixEmojiData *d = g_new0(MatrixEmojiData, 1); d->user_id = g_strdup(target_user_id); d->flow_id = g_strdup(flow_id); d->emojis = g_strdup(emojis);
...
static gboolean process_qr_cb(gpointer data) {
  ...
  if (account && purple_account_get_connection(account) != NULL) purple_notify_formatted(purple_account_get_connection(account), "QR", "Scan", NULL, d->html_data, NULL, NULL);
  g_free(d->user_id); g_free(d->html_data); g_free(d); return FALSE;
}

void show_verification_qr_cb(const char *user_id, const char *target_user_id, const char *html_data) {
  MatrixQrData *d = g_new0(MatrixQrData, 1); d->user_id = g_strdup(target_user_id); d->html_data = g_strdup(html_data);

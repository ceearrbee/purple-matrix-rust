...
static void
on_action_discover_search_members_global(PurplePluginAction *action);
...
static void emit_matrix_signal(PurpleConversation *conv,
                               const char *signal_name);
static gboolean on_activity_label_button_press_cb(GtkWidget *widget,
                                                  GdkEventButton *event,
                                                  gpointer data);

static gboolean is_conv_room_muted(PurpleConversation *conv) {
  const char *v = NULL;
  if (!conv)
    return FALSE;
...
static gboolean emit_signal_idle_cb(gpointer data) {
  ...
  if (g_list_find(purple_get_conversations(), sd->conv)) {
    ...
    PurplePlugin *matrix_plugin =
        purple_plugins_find_with_id("prpl-matrix-rust");
    if (matrix_plugin && purple_plugin_is_loaded(matrix_plugin)) {
      purple_debug_info("matrix-ui",
                        "emit_signal_idle_cb: Emitting signal %s for room %s\n",
                        sd->signal_name, room_id);
      purple_signal_emit(matrix_plugin, sd->signal_name, room_id);
    } else {
      purple_debug_warning("matrix-ui",
                           "emit_signal_idle_cb: Matrix plugin not found or "
                           "not loaded (signal %s failed)\n",
                           sd->signal_name);
    }
  }
...
static gboolean emit_global_signal_idle_cb(gpointer data) {
  GlobalSignalIdleData *sd = (GlobalSignalIdleData *)data;
  PurplePlugin *matrix_plugin = purple_plugins_find_with_id("prpl-matrix-rust");
  if (matrix_plugin && purple_plugin_is_loaded(matrix_plugin)) {
    purple_debug_info("matrix-ui",
                      "emit_global_signal_idle_cb: Emitting signal %s\n",
                      sd->signal_name);
    purple_signal_emit(matrix_plugin, sd->signal_name,
                       sd->payload ? sd->payload : "");
  } else {
    purple_debug_warning("matrix-ui",
                         "emit_global_signal_idle_cb: Matrix plugin not found "
                         "or not loaded (signal %s failed)\n",
                         sd->signal_name);
  }
  g_free(sd->signal_name);
  g_free(sd->payload);
...
static void emit_matrix_global_signal(const char *signal_name,
                                      const char *payload) {
  if (!signal_name)
    return;
...
static void emit_matrix_room_user_signal(const char *signal_name,
                                         const char *room_id,
                                         const char *user_id) {
  char *payload = NULL;
  if (!signal_name || !room_id || !*room_id || !user_id || !*user_id)
    return;
...
static void on_matrix_moderation_actions_clicked(GtkWidget *b,
                                                 PurpleConversation *c);
static void on_matrix_admin_actions_clicked(GtkWidget *b,
                                            PurpleConversation *c);
static void on_matrix_message_actions_clicked(GtkWidget *b,
                                              PurpleConversation *c);
static void on_matrix_actions_clicked(GtkWidget *b, PurpleConversation *c);

static void on_menu_dash(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-show-dashboard");
}
static void on_menu_reply(gpointer d) {
...
static void on_menu_threads(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-list-threads");
}
static void on_menu_sticker(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-sticker");
}
static void on_menu_poll(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-poll");
}
static void on_menu_clear_session(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-clear-session");
}
static void on_menu_show_members(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-show-members");
}
static void on_menu_moderate(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-moderate");
}
static void on_menu_kick(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-kick");
}
static void on_menu_ban(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-ban");
}
static void on_menu_unban(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-unban");
}
static void on_menu_leave_room(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-leave-room");
}
static void on_menu_verify_self(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-verify-self");
}
static void on_menu_crypto_status(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-crypto-status");
}
static void on_menu_list_devices(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-list-devices");
}
static void on_menu_room_settings(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-room-settings");
}
static void on_menu_invite_user(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-invite-user");
}
static void on_menu_send_file(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-send-file");
}
static void on_menu_mark_unread(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-mark-unread");
}
static void on_menu_mark_read(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-mark-read");
}
static void on_menu_mute_room(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-mute-room");
}
static void on_menu_unmute_room(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-unmute-room");
}
static void on_menu_search_room(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-search-room");
}
static void on_menu_list_polls(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-list-polls");
}
static void on_menu_react(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-react");
}
static void on_menu_send_location(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-send-location");
}
static void on_menu_who_read(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-who-read");
}
static void on_menu_report_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-report-event");
}
static void on_menu_message_inspector(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-message-inspector");
}
static void on_menu_show_last_event_details(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-show-last-event-details");
}
static void on_menu_reply_pick_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-reply-pick-event");
}
static void on_menu_thread_pick_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-thread-pick-event");
}
static void on_menu_react_pick_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-react-pick-event");
}
static void on_menu_edit_pick_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-edit-pick-event");
}
static void on_menu_redact_pick_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-redact-pick-event");
}
static void on_menu_report_pick_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-report-pick-event");
}
static void on_menu_redact_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-redact-event");
}
static void on_menu_edit_event(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-edit-event");
}
static void on_menu_react_latest(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-react-latest");
}
static void on_menu_edit_latest(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-edit-latest");
}
static void on_menu_redact_latest(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-redact-latest");
}
static void on_menu_report_latest(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-report-latest");
}
static void on_menu_versions(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-versions");
}
static void on_menu_enable_key_backup(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-enable-key-backup");
}
static void on_menu_my_profile(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-my-profile");
}
static void on_menu_server_info(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-server-info");
}
static void on_menu_resync_recent(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-resync-recent");
}
static void on_menu_search_stickers(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-search-stickers");
}
static void on_menu_recover_keys(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-recover-keys");
}
static void on_menu_export_keys(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-export-keys");
}
static void on_menu_set_avatar(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-set-avatar");
}
static void on_menu_room_tag(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-room-tag");
}
static void on_menu_upgrade_room(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-upgrade-room");
}
static void on_menu_alias_create(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-alias-create");
}
static void on_menu_alias_delete(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-alias-delete");
}
static void on_menu_space_hierarchy(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d,
                     "matrix-ui-action-space-hierarchy");
}

static void on_menu_user_info(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-user-info");
}
static void on_menu_ignore_user(gpointer d) {
  emit_matrix_signal((PurpleConversation *)d, "matrix-ui-action-ignore-user");
}
static void on_menu_unignore_user(gpointer d) {
...
static gboolean chat_user_list_button_press_cb(GtkWidget *widget,
                                               GdkEventButton *event,
                                               gpointer data) {
  PurpleConversation *conv = (PurpleConversation *)data;
  ...
  g_signal_connect_data(item, "activate",
                        G_CALLBACK(on_chat_user_action_activate), info_data,
                        (GClosureNotify)room_user_action_data_free, 0);
  ...
  g_signal_connect_data(item, "activate",
                        G_CALLBACK(on_chat_user_action_activate), mod_data,
                        (GClosureNotify)room_user_action_data_free, 0);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_data(item, "activate",
                        G_CALLBACK(on_chat_user_action_activate), ignore_data,
                        (GClosureNotify)room_user_action_data_free, 0);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_data(item, "activate",
                        G_CALLBACK(on_chat_user_action_activate), unignore_data,
                        (GClosureNotify)room_user_action_data_free, 0);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
...
static void on_matrix_moderation_actions_clicked(GtkWidget *b,
  ...
  item = gtk_menu_item_new_with_label("Ignore User...");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_ignore_user),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Unignore User...");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_unignore_user),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
...
static void on_matrix_admin_actions_clicked(GtkWidget *b,
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_server_info),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate",
                           G_CALLBACK(on_menu_enable_key_backup), c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Recover Keys");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_recover_keys),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Export Keys");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_export_keys),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_upgrade_room),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Create Alias");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_alias_create),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Delete Alias");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_alias_delete),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate",
                           G_CALLBACK(on_menu_space_hierarchy), c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
...
static void on_matrix_message_actions_clicked(GtkWidget *b,
  ...
  g_signal_connect_swapped(item, "activate",
                           G_CALLBACK(on_menu_message_inspector), c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Show Last Event Details");
  g_signal_connect_swapped(item, "activate",
                           G_CALLBACK(on_menu_show_last_event_details), c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_react_latest),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Edit Latest");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_edit_latest),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Redact Latest");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_redact_latest),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Report Latest");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_report_latest),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_redact_event),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Report Event");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_report_event),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Re-sync Recent");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_resync_recent),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Search Stickers");
  g_signal_connect_swapped(item, "activate",
                           G_CALLBACK(on_menu_search_stickers), c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
...
static void on_matrix_actions_clicked(GtkWidget *b, PurpleConversation *c) {
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_start_thread),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Show Members");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_show_members),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_invite_user),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_search_room),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_send_location),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_room_settings),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Verify This Device");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_verify_self),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Crypto Status");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_crypto_status),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("List My Devices");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_list_devices),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);

  item = gtk_menu_item_new_with_label("Mark Unread");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_mark_unread),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  ...
  if (is_conv_room_muted(c)) {
    ...
    g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_unmute_room),
                             c);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  } else {
    item = gtk_menu_item_new_with_label("Mute Room");
    g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_mute_room),
                             c);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
  }

  item = gtk_menu_item_new_with_label("Clear Session Cache");
  g_signal_connect_swapped(item, "activate", G_CALLBACK(on_menu_clear_session),
                           c);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), item);
...
static GList *ui_actions(PurplePlugin *plugin, gpointer context) {
  GList *m = NULL;
  m = g_list_append(m, purple_plugin_action_new("Login (Password)...",
                                                on_action_login_password));
  m = g_list_append(
      m, purple_plugin_action_new("Login (SSO)...", on_action_login_sso));
  m = g_list_append(m,
                    purple_plugin_action_new("Set Homeserver / Username...",
                                             on_action_set_account_defaults));
  m = g_list_append(m, purple_plugin_action_new("Discover: Join Room...",
                                                on_action_discover_join));
  m = g_list_append(m, purple_plugin_action_new("Discover: Knock Room...",
                                                on_action_discover_knock));
  m = g_list_append(
      m, purple_plugin_action_new("Discover: Browse Public Rooms...",
                                  on_action_discover_browse_public_rooms));
  m = g_list_append(m,
                    purple_plugin_action_new("Discover: Preview Room/Alias...",
                                             on_action_discover_preview_room));
  m = g_list_append(m,
                    purple_plugin_action_new("Discover: Search Users...",
                                             on_action_discover_search_users));
  m = g_list_append(m,
                    purple_plugin_action_new("Discover: Search Public Rooms...",
                                             on_action_discover_search_public));
  m = g_list_append(
      m, purple_plugin_action_new("Discover: Search Members in Room...",
                                  on_action_discover_search_members_global));
  m = g_list_append(
      m, purple_plugin_action_new("Discover: Search Messages in Room...",
                                  on_action_discover_search_room_global));
  m = g_list_append(
      m, purple_plugin_action_new("Account: My Profile", on_action_my_profile));
  m = g_list_append(m, purple_plugin_action_new("Account: Set Avatar...",
                                                on_action_set_avatar));
  m = g_list_append(
      m, purple_plugin_action_new("Server: Info", on_action_server_info));
  m = g_list_append(m, purple_plugin_action_new("Security: Recover Keys...",
                                                on_action_recover_keys));
  m = g_list_append(m, purple_plugin_action_new("Security: Export Keys...",
                                                on_action_export_keys));
  m = g_list_append(m, purple_plugin_action_new("Clear Session Cache...",
                                                on_action_clear_session));
  return m;
}

static void handle_room_typing_cb(const char *room_id, const char *who,
                                  gboolean is_typing, gpointer data) {
...
static void handle_room_encrypted_cb(const char *room_id, gboolean is_encrypted,
                                     gpointer data) {
  GList *convs = purple_get_conversations();
  for (GList *it = convs; it; it = it->next) {
    PurpleConversation *conv = (PurpleConversation *)it->data;
    ...
    if (account && strcmp(purple_account_get_protocol_id(account),
      ...
      if (strcmp(purple_conversation_get_name(conv), room_id) == 0) {
        GtkWidget *label =
            purple_conversation_get_data(conv, MATRIX_UI_ENCRYPTED_LABEL_KEY);
        if (label && GTK_IS_LABEL(label)) {
          if (is_encrypted) {
            gtk_label_set_markup(GTK_LABEL(label),
                                 "<span color='darkgreen'>🔒</span>");
            /* Use Pidgin's toolkit for the tooltip if possible, or just
             * standard GTK */
            gtk_widget_set_tooltip_text(label,
                                        "This room is End-to-End Encrypted.");
...
static void handle_room_muted_cb(const char *room_id, gboolean muted,
                                 gpointer data) {
  GList *convs = purple_get_conversations();
  for (GList *it = convs; it; it = it->next) {
    PurpleConversation *conv = (PurpleConversation *)it->data;
    ...
    if (account && strcmp(purple_account_get_protocol_id(account),
      ...
      if (cid &&
          (strcmp(cid, room_id) == 0 ||
           (g_str_has_prefix(cid, room_id) && cid[strlen(room_id)] == '|'))) {
        GtkWidget *label =
            purple_conversation_get_data(conv, MATRIX_UI_MUTED_LABEL_KEY);
        if (label && GTK_IS_LABEL(label)) {
          if (muted) {
            gtk_label_set_markup(GTK_LABEL(label),
...
static void handle_room_activity_cb(const char *room_id, const char *sender,
                                    const char *snippet, gpointer data) {
  GList *convs = purple_get_conversations();
  for (GList *it = convs; it; it = it->next) {
    PurpleConversation *conv = (PurpleConversation *)it->data;
    PurpleAccount *account = purple_conversation_get_account(conv);
    if (account && strcmp(purple_account_get_protocol_id(account),
      ...
      if (strcmp(purple_conversation_get_name(conv), room_id) == 0) {
        GtkWidget *label =
            purple_conversation_get_data(conv, MATRIX_UI_ACTIVITY_LABEL_KEY);
        if (label && GTK_IS_LABEL(label)) {
          char *msg = NULL;
          char *safe = NULL;
...
static gboolean inject_action_bar_idle_cb(gpointer data) {
  ...
  if (!g_list_find(purple_get_conversations(), conv))
    return FALSE;
  if (purple_conversation_get_data(conv, MATRIX_UI_ACTION_BAR_KEY))
    return FALSE;
  ...
  if (!gtkconv || !gtkconv->lower_hbox || !GTK_IS_WIDGET(gtkconv->lower_hbox)) {
    purple_debug_info(
        "matrix-ui",
        "inject_action_bar_idle_cb: lower_hbox not ready yet, retrying...\n");
    return TRUE;
  }

  if (!GTK_WIDGET_REALIZED(gtkconv->lower_hbox)) {
    purple_debug_info("matrix-ui", "inject_action_bar_idle_cb: lower_hbox not "
                                   "realized yet, retrying...\n");
    return TRUE;
  }

  purple_debug_info(
      "matrix-ui",
      "inject_action_bar_idle_cb: Injecting action bar into room %s\n",
  ...
  GtkWidget *poll_btn = gtk_button_new_with_mnemonic("_Poll+");
  GtkWidget *polls_btn = gtk_button_new_with_mnemonic("P_olls");
  GtkWidget *msg_btn = gtk_button_new_with_mnemonic("_Msg");
  GtkWidget *admin_btn = gtk_button_new_with_mnemonic("_Admin");
  GtkWidget *moderate_btn = gtk_button_new_with_mnemonic("M_oderate");
  GtkWidget *more_btn = gtk_button_new_with_mnemonic("M_ore");
  GtkWidget *btns[] = {reply_btn, thread_start_btn, threads_btn,
                       poll_btn,  polls_btn,        msg_btn,
                       admin_btn, moderate_btn,     more_btn};
  ...
  g_signal_connect_swapped(threads_btn, "clicked", G_CALLBACK(on_menu_threads),
                           conv);
  g_signal_connect_swapped(poll_btn, "clicked", G_CALLBACK(on_menu_poll), conv);
  g_signal_connect_swapped(polls_btn, "clicked", G_CALLBACK(on_menu_list_polls),
                           conv);
  g_signal_connect(msg_btn, "clicked",
                   G_CALLBACK(on_matrix_message_actions_clicked), conv);
  g_signal_connect(admin_btn, "clicked",
                   G_CALLBACK(on_matrix_admin_actions_clicked), conv);
  g_signal_connect(moderate_btn, "clicked",
                   G_CALLBACK(on_matrix_moderation_actions_clicked), conv);
  g_signal_connect(more_btn, "clicked", G_CALLBACK(on_matrix_actions_clicked),
                   conv);
  ...
  gtk_widget_set_tooltip_text(activity_event_box,
                              "Left click: show last event details. Right "
                              "click: open Message Inspector.");
  ...
  purple_conversation_set_data(conv, MATRIX_UI_ACTIVITY_LABEL_KEY,
                               activity_label);
...
static void conversation_extended_menu_cb(PurpleConversation *conv,
                                          GList **list) {
  PurpleAccount *account = purple_conversation_get_account(conv);
  if (account && strcmp(purple_account_get_protocol_id(account),
                        "prpl-matrix-rust") == 0) {
    PurpleMenuAction *matrix_menu =
        purple_menu_action_new("Matrix", NULL, NULL, NULL);
    GList *sub_actions = NULL;
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Room Dashboard", PURPLE_CALLBACK(on_menu_dash),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
    ...
    sub_actions = g_list_append(
        ...
        purple_menu_action_new("Reply (Pick Event)",
                               PURPLE_CALLBACK(on_menu_reply_pick_event), conv,
                               NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Start Thread", PURPLE_CALLBACK(on_menu_start_thread), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Start Thread (Pick Event)",
                               PURPLE_CALLBACK(on_menu_thread_pick_event), conv,
                               NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("View Threads", PURPLE_CALLBACK(on_menu_threads),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Send Sticker", PURPLE_CALLBACK(on_menu_sticker),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Create Poll", PURPLE_CALLBACK(on_menu_poll),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("User Info", PURPLE_CALLBACK(on_menu_user_info),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Ignore User...", PURPLE_CALLBACK(on_menu_ignore_user),
                         conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Unignore User...",
                         PURPLE_CALLBACK(on_menu_unignore_user), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Moderate Room",
                               PURPLE_CALLBACK(on_menu_moderate), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Kick User...", PURPLE_CALLBACK(on_menu_kick),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Ban User...", PURPLE_CALLBACK(on_menu_ban),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Unban User...", PURPLE_CALLBACK(on_menu_unban),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Show Members", PURPLE_CALLBACK(on_menu_show_members), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Invite User", PURPLE_CALLBACK(on_menu_invite_user), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Send File", PURPLE_CALLBACK(on_menu_send_file),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Search in Room", PURPLE_CALLBACK(on_menu_search_room),
                         conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Message Inspector...",
                               PURPLE_CALLBACK(on_menu_message_inspector), conv,
                               NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Show Last Event Details",
                               PURPLE_CALLBACK(on_menu_show_last_event_details),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Re-sync Recent",
                         PURPLE_CALLBACK(on_menu_resync_recent), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new("List Polls",
                                            PURPLE_CALLBACK(on_menu_list_polls),
                                            conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Add Reaction", PURPLE_CALLBACK(on_menu_react),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("React (Pick Event)",
                               PURPLE_CALLBACK(on_menu_react_pick_event), conv,
                               NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Search Stickers",
                         PURPLE_CALLBACK(on_menu_search_stickers), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "React to Latest",
                         PURPLE_CALLBACK(on_menu_react_latest), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Send Location",
                         PURPLE_CALLBACK(on_menu_send_location), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Who Read", PURPLE_CALLBACK(on_menu_who_read),
                               conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Report Event", PURPLE_CALLBACK(on_menu_report_event), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Report (Pick Event)",
                               PURPLE_CALLBACK(on_menu_report_pick_event), conv,
                               NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new("Edit Event",
                                            PURPLE_CALLBACK(on_menu_edit_event),
                                            conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Edit (Pick Event)",
                         PURPLE_CALLBACK(on_menu_edit_pick_event), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Redact Event", PURPLE_CALLBACK(on_menu_redact_event), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Redact (Pick Event)",
                               PURPLE_CALLBACK(on_menu_redact_pick_event), conv,
                               NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Edit Latest", PURPLE_CALLBACK(on_menu_edit_latest), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Redact Latest",
                         PURPLE_CALLBACK(on_menu_redact_latest), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Report Latest",
                         PURPLE_CALLBACK(on_menu_report_latest), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Server Versions",
                               PURPLE_CALLBACK(on_menu_versions), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Server Info", PURPLE_CALLBACK(on_menu_server_info), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new("My Profile",
                                            PURPLE_CALLBACK(on_menu_my_profile),
                                            conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new("Set Avatar",
                                            PURPLE_CALLBACK(on_menu_set_avatar),
                                            conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Enable Key Backup",
                               PURPLE_CALLBACK(on_menu_enable_key_backup), conv,
                               NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Recover Keys", PURPLE_CALLBACK(on_menu_recover_keys), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Export Keys", PURPLE_CALLBACK(on_menu_export_keys), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Set Room Tag",
                               PURPLE_CALLBACK(on_menu_room_tag), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Upgrade Room", PURPLE_CALLBACK(on_menu_upgrade_room), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Create Alias", PURPLE_CALLBACK(on_menu_alias_create), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Delete Alias", PURPLE_CALLBACK(on_menu_alias_delete), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Space Hierarchy",
                         PURPLE_CALLBACK(on_menu_space_hierarchy), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Room Settings",
                         PURPLE_CALLBACK(on_menu_room_settings), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Verify This Device",
                         PURPLE_CALLBACK(on_menu_verify_self), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Crypto Status",
                         PURPLE_CALLBACK(on_menu_crypto_status), conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "List My Devices",
                         PURPLE_CALLBACK(on_menu_list_devices), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new(
            "Mark Unread", PURPLE_CALLBACK(on_menu_mark_unread), conv, NULL));
    sub_actions = g_list_append(
        sub_actions,
        purple_menu_action_new("Mark Read", PURPLE_CALLBACK(on_menu_mark_read),
                               conv, NULL));
    if (is_conv_room_muted(conv)) {
      sub_actions = g_list_append(
          sub_actions,
          purple_menu_action_new(
              "Unmute Room", PURPLE_CALLBACK(on_menu_unmute_room), conv, NULL));
    } else {
      sub_actions = g_list_append(
          sub_actions,
          purple_menu_action_new(
              "Mute Room", PURPLE_CALLBACK(on_menu_mute_room), conv, NULL));
    }
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new("Leave Room",
                                            PURPLE_CALLBACK(on_menu_leave_room),
                                            conv, NULL));
    sub_actions = g_list_append(
        sub_actions, purple_menu_action_new(
                         "Clear Session Cache",
                         PURPLE_CALLBACK(on_menu_clear_session), conv, NULL));
...
static void connect_ui_signals(PurplePlugin *plugin) {
  ...
  if (matrix_plugin) {
    purple_signal_connect(matrix_plugin, "matrix-ui-room-typing", plugin,
                          PURPLE_CALLBACK(handle_room_typing_cb), NULL);
    purple_signal_connect(matrix_plugin, "matrix-ui-room-encrypted", plugin,
                          PURPLE_CALLBACK(handle_room_encrypted_cb), NULL);
    purple_signal_connect(matrix_plugin, "matrix-ui-room-muted", plugin,
                          PURPLE_CALLBACK(handle_room_muted_cb), NULL);
    purple_signal_connect(matrix_plugin, "matrix-ui-room-activity", plugin,
                          PURPLE_CALLBACK(handle_room_activity_cb), NULL);
  }
...
static gboolean plugin_unload(PurplePlugin *plugin) {
  ...
  for (GList *it = purple_get_conversations(); it != NULL; it = it->next) {
    PurpleConversation *conv = (PurpleConversation *)it->data;
    GtkWidget *bar =
        purple_conversation_get_data(conv, MATRIX_UI_ACTION_BAR_KEY);
...
static PurplePluginInfo info = {
    ...
    .version = "0.3.0",
    .summary = "Modern Action Bar with Typing & Encryption info.",
    .description = "Adds real-time typing indicators, encryption status, and "
